...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  if (info.menuItemId === 'copyXPath') {
    await handleXPathExtraction(info, tab, false);
  } else if (info.menuItemId === 'copyXPathWithAI') {
    await handleXPathExtraction(info, tab, true);
  }
});

// 统一处理XPath提取逻辑
async function handleXPathExtraction(info, tab, useAI) {
  try {
    // 首先检查内容脚本是否已加载
    ...
    if (response && response.loaded) {
      ...
      await chrome.tabs.sendMessage(tab.id, {
        action: useAI ? 'generateXPathWithAI' : 'generateXPath',
        data: {
          x: info.x || 0,
          y: info.y || 0
        }
