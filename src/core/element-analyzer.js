...
class ElementAnalyzer {
  ...
  analyze(element, options = {}) {
    ...
    const analysis = this.performAnalysis(element, options);
  ...
  performAnalysis(element, options = {}) {
    return {
      ...
      mode: options.useAI ? 'ai' : 'traditional'
    };
  }
  ...
  clearCache() {
    ...
    this.cache.clear();
    console.log(`元素分析器清空了 ${size} 个缓存项`);
  }
  ...
  clearCacheByMode(mode) {
    ...
    console.log(`元素分析器清理了 ${cleanedCount} 个 ${mode === 'ai' ? 'AI模式' : '传统模式'}缓存项`);
  }
