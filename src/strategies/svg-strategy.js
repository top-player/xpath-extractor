    ...
    try {
      ...
      for (const xpath of strategies) {
        if (this.validateXPath(xpath, svgElement)) {
          console.log('SVG策略成功生成XPath:', xpath);
          return xpath;
        }
      }
      
      // 如果所有策略都失败，返回基于位置的定位
      const positionalXPath = this.generatePositionalXPath(svgElement);
      console.log('SVG策略使用位置定位:', positionalXPath);
      return positionalXPath;
      
    } catch (error) {
      console.error('SVG策略执行过程中出错:', error);
      return null;
    }
  }
